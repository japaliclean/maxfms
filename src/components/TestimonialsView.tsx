...
export default function TestimonialsView() {
  ...
  useEffect(() => {
    ...
    const width = filteredTestimonials.length > 1
      ? `${Math.round((activeIndex / (filteredTestimonials.length - 1)) * 100)}%`
      : '0%';
    progressBarRef.current.style.width = width;
  }, [activeIndex, filteredTestimonials.length]);
    ...
    >
      ...
      <motion.section variants={fadeInUp} className="max-w-container-max mx-auto pb-20">
        ...
        <div className="relative overflow-hidden rounded-[32px] border border-zinc-200 bg-zinc-950/5 p-4">
          ...
          <div
            ref={galleryRef}
            className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-none touch-pan-x"
          ...
          >
            {filteredTestimonials.map((item) => (
              <button
                ...
                aria-label={`View gallery item for ${item.name}`}
          ...
          <div className="mt-4 h-2 rounded-full bg-zinc-200/60 overflow-hidden">
            <div
              ref={progressBarRef}
              className="h-full rounded-full bg-primary transition-all duration-300 gallery-progress-fill"
            />
          </div>
        </div>
        </div>
